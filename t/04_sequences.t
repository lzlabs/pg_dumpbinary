...
ok( $? == 0 && `ls t/test_bin_dump/ | wc -l` == 5, "Dump test database for sequence namespace");
...
ok( $? == 0 && `ls t/test_bin_dump/ | wc -l` == 5, "Dump test sequence export with filter on tables");
ok( `grep "test_seq[12]_id_seq.5" t/test_bin_dump/data-sequences.lst | wc -l` == 2, "Number of sequence exported");
`rm -rf t/test_bin_dump`;
