...
ok( $? == 0 && `ls t/test_bin_dump/ | wc -l` == 14, "Dump test database for single namespace");
`rm -rf t/test_bin_dump`;
$ret = `perl pg_dumpbinary -n '"BTEST"' -t '"BTEST"."T1"' -d test_bin_dump_orig t/test_bin_dump`;
ok( $? == 0 && `ls t/test_bin_dump/ | wc -l` == 4, "Dump test database for single table");
`rm -rf t/test_bin_dump`;
$ret = `perl pg_dumpbinary -n '"BTEST"' -T '"BTEST"."T1"' -d test_bin_dump_orig t/test_bin_dump`;
ok( $? == 0 && `ls t/test_bin_dump/ | wc -l` == 12, "Dump test database excluding single table");
`rm -rf t/test_bin_dump`;
